...
def processAlns(fOut, qname, arr, label, asDiff, counts):
  '''
...
def processProper(fOut, qname, R1, R2, asDiff, counts):
  '''
  ...
  for r in R2:
    ...
    if not r[0] and r[2]:
      ...
      if r[3]:
        ...
        for s in alns:
          if r[4] == s[4] and r[5] == s[6] and r[6] == s[5] and not s[8]:
            s[8] = True
            s[7] += r[7]
            mat = True
  ...
  processAlns(fOut, qname, paired, 'PE', asDiff, counts)

def processRead(fOut, qname, lines, minMapQ, asDiff, counts):
  '''
  ...
  for spl in lines:
    ...
    if flag & 0x40 or not pe:
      R1.append([flag & 0x4, int(spl[4]) < minMapQ, flag & 0x3 == 0x3,
        not(flag & 0xF00), spl[2], spl[3], spl[7], AS])
    else:
      R2.append([flag & 0x4, int(spl[4]) < minMapQ, flag & 0x3 == 0x3,
        not(flag & 0xF00), spl[2], spl[3], spl[7], AS])

  if proper:
    processProper(fOut, qname, R1, R2, asDiff, counts)
  else:
    if not pe:
      processAlns(fOut, qname, R1, 'SE', asDiff, counts)
    else:
      processAlns(fOut, qname, R1, 'R1', asDiff, counts)
      processAlns(fOut, qname, R2, 'R2', asDiff, counts)


def parseSAM(fIn, fOut, line, minMapQ, asDiff, verbose):
  '''
  ...
  while line:
    ...
    if spl[0] == qname:
      ...
      processRead(fOut, qname, lines, minMapQ, asDiff, counts)
      counts[0] += 1
...
def main():
  ...
  # check SAM header
  line, count = loadHeader(fIn, args.unsorted)
  if args.verbose:
    sys.stderr.write('Reference sequences in SAM: %d\n' % count)

  # load info from SAM
  counts = parseSAM(fIn, fOut, line, args.minMapQ, args.asDiff,
    args.verbose)
