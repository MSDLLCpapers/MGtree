process FASTP {
    ...
    def r1
    def r2
    if (meta.single_end || task.ext.args?.contains('--interleaved_in')) {
        r1 = reads.join(' ')
        r2 = []
    }
    else {
        (r1, r2) = reads.collate(2).transpose().collect { r -> r.join(' ') }
    }
    // Added soft-links to original fastqs for consistent naming in MultiQC
    // Use single ended for interleaved. Add --interleaved_in in config. 
    if (task.ext.args?.contains('--interleaved_in')) {
        """
        [ ! -f  ${prefix}.fastq.gz ] && ${reads.size() == 1 ? "ln -sf ${r1}" : " cat ${r1} >"} ${prefix}.fastq.gz
        ...
        """
        [ ! -f  ${prefix}_1.fastq.gz ] && ${reads.size() == 2 ? "ln -sf ${r1}" : "cat ${r1} >"} ${prefix}_1.fastq.gz
        [ ! -f  ${prefix}_2.fastq.gz ] && ${reads.size() == 2 ? "ln -sf ${r2}" : "cat ${r2} >"} ${prefix}_2.fastq.gz
        fastp \\
