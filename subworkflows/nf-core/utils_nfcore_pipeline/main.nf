...
def paramsSummaryMultiqc(summary_params) {
    def summary_section = summary_params
        .keySet()
        .collect { group ->
            def summary_section_part = ''
            def group_params = summary_params.get(group)
            // This gets the parameters of that particular group
            if (group_params) {
                summary_section_part += "    <p style=\"font-size:110%\"><b>${group}</b></p>\n"
                summary_section_part += "    <dl class=\"dl-horizontal\">\n"
                summary_section_part += group_params
                    .keySet()
                    .sort()
                    .collect { param ->
                        "        <dt>${param}</dt><dd><samp>${group_params.get(param) ?: '<span style=\"color:#999999;\">N/A</a>'}</samp></dd>\n"
                    }
                    .join('')
                summary_section_part += "    </dl>\n"
            }
            summary_section_part
        }
        .join('')
...
//
def completionEmail(summary_params, email, email_on_fail, plaintext_email, outdir, monochrome_logs = true, multiqc_report = null, max_multiqc_email_size = 0.GB) {
    ...
    if (email_address) {
        ...
        log.info("-${colors.purple}[${workflow.manifest.name}]${colors.green} Sent summary e-mail to ${email_address} (sendMail)${colors.reset}-")
